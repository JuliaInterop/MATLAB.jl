...
data_ptr(mx::MxArray) = convert(Ptr{eltype(mx)}, @mxget_attr(mx_get_data[], Ptr{Void}, mx))
real_ptr(mx::MxArray) = convert(Ptr{eltype(mx)}, @mxget_attr(mx_get_pr[], Ptr{Void}, mx))
imag_ptr(mx::MxArray) = convert(Ptr{eltype(mx)}, @mxget_attr(mx_get_pi[], Ptr{Void}, mx))
...
function mxarray(x::Float64)
    pm = ccall(mx_create_double_scalar[], Ptr{Void}, (Cdouble,), x)
    MxArray(pm)
end

function mxarray(x::Bool)
    pm = ccall(mx_create_logical_scalar[], Ptr{Void}, (Bool,), x)
    MxArray(pm)
end
...
function mxarray{T<:MxRealNum}(x::T)
    ...
    pdat = ccall(mx_get_data[], Ptr{T}, (Ptr{Void},), pm)
...
function mxarray{V<:Union{Float64,Bool},I}(a::SparseMatrixCSC{V,I})
    ...
    ir_p = ccall(mx_get_ir[], Ptr{mwIndex}, (Ptr{Void},), mx)
    jc_p = ccall(mx_get_jc[], Ptr{mwIndex}, (Ptr{Void},), mx)
    pr_p = ccall(mx_get_pr[], Ptr{V}, (Ptr{Void},), mx)
...
function mxarray(s::String)
    ...
    pm = ccall(mx_create_char_array[], Ptr{Void}, (mwSize, Ptr{mwSize},), 2,
...
function mxcellarray(dims::Tuple{Vararg{Int}})
    pm = ccall(mx_create_cell_array[], Ptr{Void}, (mwSize, Ptr{mwSize}),
...
function get_cell(mx::MxArray, i::Integer)
    pm = ccall(mx_get_cell[], Ptr{Void}, (Ptr{Void}, mwIndex), mx, i-1)
    MxArray(pm, false)
end

function set_cell(mx::MxArray, i::Integer, v::MxArray)
    v.own = false
    ccall(mx_set_cell[], Void, (Ptr{Void}, mwIndex, Ptr{Void}), mx, i - 1, v)
    return nothing
end
...
function mxstruct(fns::Vector{String})
    ...
    pm = ccall(mx_create_struct_matrix[], Ptr{Void},
...
function set_field(mx::MxArray, i::Integer, f::String, v::MxArray)
    v.own = false
    ccall(mx_set_field[], Void, (Ptr{Void}, mwIndex, Ptr{UInt8}, Ptr{Void}), mx, i-1, f, v)
    return nothing
end
...
function get_field(mx::MxArray, i::Integer, f::String)
    pm = ccall(mx_get_field[], Ptr{Void}, (Ptr{Void}, mwIndex, Ptr{UInt8}), mx, i-1, f)
...
function get_field(mx::MxArray, i::Integer, fn::Integer)
    pm = ccall(mx_get_field_bynum[], Ptr{Void}, (Ptr{Void}, mwIndex, Cint), mx, i-1, fn-1)
    pm == C_NULL && throw(ArgumentError("Failed to get field."))
    MxArray(pm, false)
end
...
function get_fieldname(mx::MxArray, i::Integer)
    p = ccall(mx_get_fieldname[], Ptr{UInt8}, (Ptr{Void}, Cint), mx, i-1)
    unsafe_string(p)
end
...
function mxstructarray{T}(d::Array{T})
    ...
    pm = ccall(mx_create_struct_array[], Ptr{Void}, (mwSize, Ptr{mwSize}, Cint,
...
function _jsparse{T<:MxRealNum}(ty::Type{T}, mx::MxArray)
    ...
    jc_ptr = ccall(mx_get_jc[], Ptr{mwIndex}, (Ptr{Void},), mx)
    pr_ptr = ccall(mx_get_pr[], Ptr{T}, (Ptr{Void},), mx)
